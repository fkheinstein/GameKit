...
namespace fts
{
    ...
    static inline MaterialType ConvertAssimpTextureType(aiTextureType textureType)
    {
        ...
        default:
            FTS_CORE_WARN("[ConvertAssimpTextureType] invalid texture type!");
    ...
    static void ProcessNode(const aiScene *scene, const aiNode *ai_node,  Model &model, ModelNode *parent)
    {
        if (ai_node == nullptr)
            return;
    ...
    {
        Ref<Model> model;
        FTS_CORE_TRACE("Loading model form: {}...", path);

        Assimp::Importer importer;
        uint32_t import_flags =   aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_CalcTangentSpace;
        const aiScene *scene = importer.ReadFile(path.data(), import_flags);
        if (scene == nullptr)
        {
            throw FileException(path, fmt::format("Model loading failed: {}",   importer.GetErrorString()));
        }
        else
        {
