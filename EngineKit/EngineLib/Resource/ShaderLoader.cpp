...
static void ParseShaderCode(const std::string& path, std::string& code)
{
    ...
    FTS_CORE_TRACE("Building shader code from {}", path);
    File::Read(path, code);
    // insert include code
...
Ref<Shader> ShaderLoader::Load(const std::string& vert_path, const std::string& frag_path)
{
    Ref<Shader> shader = Shader::Create();
...
Ref<Shader> ShaderLoader::Load(const std::string& vert_path, const std::string& frag_path,  const std::string& geo_path)
{
    Ref<Shader> shader = Shader::Create();
    std::string vert_code;
    std::string frag_code;
