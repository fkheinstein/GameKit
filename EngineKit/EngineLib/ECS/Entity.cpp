...
namespace fts
{
    ...
    {
        if (*this == child) {
            FTS_CORE_WARN("Cannot add self as child!");
            return;
        };
        auto& data = GetComponent<TransformComponent>();
        auto& child_data = child.GetComponent<TransformComponent>();
        if (data.parent == child) {
            FTS_CORE_WARN("Cannot add parent as child!");
            return;
        }
        if (child_data.parent == *this) {
            FTS_CORE_WARN("This entity already has this child!");
            return;
        }
    ...
    void Entity::RemoveChild(Entity& child)
    {
        ...
        if (iter != children.end()) {
            ...
            FTS_CORE_WARN("Entity cannot find specified child!");
        }
        auto& child_data = child.GetComponent<TransformComponent>();
