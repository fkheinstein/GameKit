...
namespace fts
{
    ...
    void Renderer::SetRenderState(Framebuffer* framebuffer, int32_t viewport_width,  int32_t viewport_height,  const RenderOperation& op)
    {
        s_device->SetFramebuffer(framebuffer);
        s_device->SetViewport(0, 0, viewport_width, viewport_height);

        op.depth_test ? s_device->Enable(Operation::DepthTest)  : s_device->Disable(Operation::DepthTest);
        s_device->SetDepthMask(op.depth_mask);

        s_device->SetDepthfunc(op.depth_func);
        op.blend ? s_device->Enable(Operation::Blend) : s_device->Disable(Operation::Blend);

        op.face_culling ? s_device->Enable(Operation::FaceCulling) : s_device->Disable(Operation::FaceCulling);
    ...
    void Renderer::BeginRenderPass(const RenderPassInfo& info)
    {
        ...
        SetRenderState(info.framebuffer, info.viewport_width, info.viewport_height,  info.op);

        s_device->SetClearColor(info.clear_value[0], info.clear_value[1], info.clear_value[2], info.clear_value[3]);
    ...
    {
        FTS_ASSERT_MSG(s_data.is_subpass_begin == false, "Nested render subpass is not allowed!");
        s_data.is_subpass_begin = true;

        // restore render pass state
        auto& render_pass = GetCurrentRenderPass();
        SetRenderState(render_pass.framebuffer, render_pass.viewport_width,  render_pass.viewport_height, info.op);

        s_device->DrawBuffers(render_pass.framebuffer, info.draw_buffer_count, info.draw_buffer);
    }
    ...
    void Renderer::EndRenderPass()
    {
        ...
        if (!IsEmptyStack()) {
            ...
            SetRenderState(render_pass.framebuffer, render_pass.viewport_width, render_pass.viewport_height, render_pass.op);
        }
    }
    ...
    void Renderer::Init(Device* device)
    {
        ...
        FTS_CORE_TRACE("Initializing Renderer");
        s_data.camera_ubo = UniformBuffer::Create(nullptr, sizeof(CameraData),   BufferIOType::Dynamic);
        ...
        // NDC quad
        {
            ...
            s_data.quad_vbo = VertexBuffer::Create(quad_vert, sizeof(quad_vert),  BufferIOType::Static);
    ...
    void Renderer::SetCamera(Camera& camera)
    {
        ...
        s_data.camera_data.view = camera.GetViewMatrix();
        s_data.camera_data.projection = camera.GetProjectionMatrix();
        ...
        int32_t depth)
    {
        ...
        s_device->SetShader(&shader);
        s_device->DispatchCompute(std::ceil(width / local_group_size.x),  std::ceil(height / local_group_size.y),  std::ceil(depth / local_group_size.z));
    ...
    {
        Submit(shader, *s_data.quad_vao, MeshTopology::Triangles,  s_data.quad_ibo->GetCount(), 0);
    }

    void Renderer::DrawNDCBox(const Shader& shader)
    {
        Submit(shader, *s_data.box_vao, MeshTopology::Triangles,  s_data.box_ibo->GetCount(), 0);
    }
