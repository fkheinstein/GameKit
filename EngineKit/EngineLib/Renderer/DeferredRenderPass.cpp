...
namespace fts {
    ...
    void DeferredRenderPass::InitSSAOKernel()
    {
        ...
        for (uint32_t i = 0; i < kernel_size; ++i) {
            Vector3f sample(Random::Rnd(-1.f, 1.f), Random::Rnd(-1.f, 1.f), Random::Rnd(0.f, 1.0f));
    ...
    void DeferredRenderPass::DrawTexture(const fts::Texture& texture, const ImVec2& uv0, const ImVec2& uv1)
    {
    ...
    void DeferredRenderPass::ImGui()
    {
        if (ImGui::TreeNodeEx("Geometry Buffers")) {
            for (size_t i = 0; i < s_data.gbuffer.size(); ++i) {
                DrawTexture(*s_data.gbuffer[i], ImVec2(0, 1), ImVec2(1, 0));
            }
            ImGui::TreePop();
        }
        if (ImGui::TreeNodeEx("SSAO")) {
            ...
            DrawTexture(*s_data.ssao_blur_buffer, ImVec2(0, 1), ImVec2(1, 0));
            ImGui::TreePop();
        }
        if (ImGui::TreeNodeEx("Cascade Shadow")) {
            ImGui::InputInt("Layer", &s_data.debug_layer);
            DrawTexture(*s_data.cascade_debug_buffer, ImVec2(0, 1), ImVec2(1, 0));
            ImGui::TreePop();
        }
        ...
        const Transform& transform)
    {
        ...
        // debug
        {
            ...
            s_data.cascade_debug_shader->GetParam("depthMap")->SetAsTexture(depth_map);
            s_data.cascade_debug_shader->GetParam("layer")->SetAsInt(s_data.debug_layer);
            s_data.cascade_debug_shader->GetParam("near_plane")->SetAsFloat(camera.GetNearZ());
            s_data.cascade_debug_shader->GetParam("far_plane")->SetAsFloat(camera.GetFarZ());
    ...
    void DeferredRenderPass::RenderGBuffer(const Scene& scene)
    {
        ...
        info.clear_mask =   BufferBitMask::ColorBufferBit | BufferBitMask::DepthBufferBit;
